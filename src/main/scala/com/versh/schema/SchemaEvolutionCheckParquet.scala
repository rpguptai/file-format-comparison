...
object SchemaEvolutionCheckParquet extends App {
  ...
  df.write.format("parquet").mode(SaveMode.Overwrite)
    .save("c:\\out\\parquet\\partition-date=2020-01-02")
  ...
  df1.write.format("parquet").mode(SaveMode.Overwrite)
    .save("c:\\out\\parquet\\partition-date=2020-01-03")


  val df2 = spark.read.options(Map("mergeSchema"->"true")).parquet("c:\\out\\parquet\\")
  df2.printSchema()
  df2.show()
}
